...
TextResearchFix:="|<>*127$45.zzzzzzxsDzzzzzjRzzzzzxvaAQMOlVRgjNvAwU84kAPDgPDkT/NxXAz9vPbgQlXX3SBY"
TextResearch1:="|<>*129$74.TzzzzzzzzzzDrzzzzzzzzzznxV67KAn64QMMFNBYnNhgYqmHBqnPhrPM/BUgnBgqvRq4ynHvAsPBYrNnbgqSnD6nMRswwPBlgt7zyzzzzzzzzzzzzjzzzzzzzzzs"
TextResearch2:="|<>*125$53.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzyCyzzRzzzvBzzyzzzzrMP68oYMzCqqZvhO7zRhc3rOUXynPPriljryCClXBr6Dzzxzzzzzzzznzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzs"
TextResearch3:="|<>*127$69.lzzTzPzy7zzwzzvzvzzqzzzjX2A3P4SqAFVx/Pj/QdkoCbj1PRVP8Cg4UZxvPd/PjqrutlXPBVO4SmAFbzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzlzzzxzxrzzzxbzzzzzjzzzzil2ABW8qAATtquwhiQiohDzir65ha5opgzxavYhhYiohvzlr25h24qBgTzznTzzzzzzzzzz7zzzzzzzzw"
TextResearchScroll:="|<>*140$14.zDznz03k0w0D7X1s0S333k0w0D03zDzny"
...
TextConquest:="|<>*120$57.kzzzzzzzyQrzzzzzzznDsQ7UnCC41yNaNaNarnTrAnQnBmSNytaPaNUMnDrAnQnBzmQqNaNaNbqHksQnUsC677zzzzbzzzzzzzzwzzzzzzzzzbzzzzU"
...
Loop {
	...
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	SendEvent {Click 1365 523 Down}{Click 699 548 Up}
	Sleep 25
	MouseMove, 1000, 900
	Goto Res4
}
...
Kazak2:
Loop {
	if ok:=FindText(990, 537, 1142, 591, 0, 0, TextKazak)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	MouseMove, %X%, %Y%
...
Kazak3:
Loop {
	if ok:=FindText(762, 402, 1034, 508, 0, 0, TextKazak2)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	Sleep 5
...
Kazak4:
Loop {
	if ok:=FindText(1046-150000, 622-150000, 1046+150000, 622+150000, 0, 0, TextJustify)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	MouseMove, %X%, %Y%
...
Kazak5:
Loop {
	if ok:=FindText(878, 524, 1034, 592, 0, 0, TextConquest)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	MouseMove, %X%, %Y%
...
Kazak6:
Loop {
	if ok:=FindText(798, 612, 1126, 660, 0, 0, TextVerify)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	Goto Kazak7
...
Kazak7:
Loop {
	if ok:=FindText(962, 644, 1158, 716, 0, 0, TextJustWar)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	Y -= 5
...
Taiwan2:
Loop {
	if ok:=FindText(992, 541, 1136, 657, 0, 0, TextTaiwan)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	MouseMove, %X%, %Y%
...
Taiwan3:
Loop {
	if ok:=FindText(760, 405, 1150, 469, 0, 0, TextTaiwan2)
    {
	X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
	;------------------------------
	Sleep 5
...
CaptureCursor(hDC:=0, zx:=0, zy:=0, zw:=0, zh:=0, get_cursor:=0)
{
  ...
  VarSetCapacity(mi, 40, 0), NumPut(16+PtrSize, mig, "int")
...
Check_ForUpdate(_ReplaceCurrentScript = 1, _SuppressMsgBox = 0, _CallbackFunction = "", ByRef _Information = "")
{
	...
	;[Info]
	;Version=1.5
	;URL=https://raw.githubusercontent.com/Guestman2/RoN_Automizer/main/AutoRoN.ahk or .exe
	;MD5=00000000000000000000000000000000 or omit this key completly to skip the MD5 file validation
	
	Static Script_Name := "autoup" ;Your script name
	, Version_Number := 1.5 ;The script's version number
	, Update_URL := "https://raw.githubusercontent.com/Guestman2/RoN_Automizer/main/Version.ini" ;The URL of the version.ini file for your script
